...
static int detachall(void)
{
  ...
  errno = 0;
  ptrace(PTRACE_CONT, thePid, 1, 0);
  if (errno) perror("continue");

  if (threads.found) {
    for (i = 0; i < threads.npids; i++) {
      if (threads.pids[i] != thePid && threads.attached[i]) {
        errno = 0;
        ptrace(PTRACE_CONT, threads.pids[i], 1, 0);
        if (errno) perror("continue");
        errno = 0;
...
{
  const Elf32_Sym *handles, *dad, *mgr;
  int i, tpid, npf, hsize, descOff, pidOff, numPids, *pptr;
  Elf32_Addr descr;

  if (!findLocalSym("__pthread_threads_debug", syms) ||
      !(handles = findLocalSym("__pthread_handles", syms)) ||
      !(dad = findLocalSym("__pthread_initial_thread", syms)) ||
      !(mgr = findLocalSym("__pthread_manager_thread", syms)) ||
  ...
  threads.pids[0] = ptrace(PTRACE_PEEKDATA, pid, dad->st_value + pidOff, 0);
  if (!errno)
    threads.pids[1] = ptrace(PTRACE_PEEKDATA, pid, mgr->st_value + pidOff, 0);

  for (pptr = &threads.pids[2], i = npf = 0; npf < numPids && !errno; i++) {
    descr = ptrace(PTRACE_PEEKDATA, pid,
                   handles->st_value + (i * hsize) + descOff, 0);
    if (descr && !errno) {
      tpid = ptrace(PTRACE_PEEKDATA, pid, descr + pidOff, 0);
      npf++;
      if (!errno && tpid != threads.pids[0] && tpid != threads.pids[1])
        *pptr++ = tpid;
    }
  }
  ...
  for (i = 0; i < threads.npids; i++) {
    if (i != 1 && threads.pids[i] && threads.pids[i] != pid) {
...
static void symAddress(Elf32_Addr addr, char *buf)
{
  Elf32_Sym *sym;
  Symbols syms;
  ...
  if (!sym) strcpy(buf, "????");
    ...
    strcpy(buf, &syms->strings[sym->st_name]);
    if (sym->st_value < addr) {
      strcat(buf, " + ");
      sprintf(buf + strlen(buf), "0x%x", addr - sym->st_value);
    }
  }
}
        ...
        for (i = 0; i < threads.npids; i++) {
          if (threads.attached[i]) {
            ...
            if (crawl(threads.pids[i]) != 0)
              fprintf(stderr, "Error dancing through thread %d\n",
                      threads.pids[i]);
          }
        }
      } else if (crawl(thePid) != 0)
        fprintf(stderr, "Error dancing through process %d\n", thePid);
    }
