...
malloc_atfork(size_t sz, const void *caller)
{
  ...
  if(vptr == ATFORK_ARENA_PTR) {
	  // dyc: this thread is calling fork 
	  ...
	  // dyc: wait other thread finish fork
...
ptmalloc_init (void)
{
  ...
  // dyc: seems a "void" define from C code, but may be a assembly code to create per-thread var
  ...
  if (__builtin_expect (_environ != NULL, 1))
    {
	  ...
	  // parse enviroment variables and call mALLOPt()
