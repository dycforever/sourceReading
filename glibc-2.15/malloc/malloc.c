...
//  typedef struct malloc_state *mstate;
  ...
  // dyc: look up in per-thread var for ptr; [arena_lookup] is defined in arena.h as below
  arena_lookup(ar_ptr);
  // void *vptr = NULL; \
  ...
  //
  // from: ../sysdeps/mach/hurd/bits/libc-tsd.h
  ...
  //
  // dyc: I think the function[ __hurd_threadvar_location() ] above is implemented by assembly language.
  //      It is used to get the per-thread variable by index [_HURD_THREADVAR_##KEY]
  //      There are some standary per-thread variables. They can be found in "../hurd/hurd/threadvar.h"
  //
...
_int_malloc(mstate av, size_t bytes)
{
  ...
  */
  ...
  if ((unsigned long)(nb) <= (unsigned long)(get_max_fast ())) {
	  ...
	  // dyc: atomically store victim->fb in *fb if *fb == victim
